...
class UpdateOfferFormWidget extends StatefulWidget {
  const UpdateOfferFormWidget(
      {super.key, required this.types, required this.offer});
  final List<HoneyType> types;
  final Offer offer;
  @override
  State<UpdateOfferFormWidget> createState() => _UpdateOfferFormWidgetState();
}

class _UpdateOfferFormWidgetState extends State<UpdateOfferFormWidget> {
  ...
  void initState() {
    ...
    for (var e in widget.offer.typesId) {
      final type = widget.types.where((x) => x.id == e);
      res.addAll(type);
    }
    ...
    discountController.text = widget.offer.discount.toString();
    selectedTypes = res;
    numberOfJar = widget.offer.quantity;
  }
                          ...
                          onPressed: () {
                            ...
                            if (_globalKey.currentState!.validate()) {
                              if (selectedTypes.isNotEmpty) {
                                print(
                                    'number of types = ${selectedTypes.length}');
                                print(
                                    'number of quantity = ${numberOfJar.length}');
                            ...
                            child: Row(
                              children: [
                                ...
                                Text(
                                  ...
                                  style: TextStyle(
                                      fontSize: 16.spMax, color: Colors.blue),
                                )
                              ],
                            ),
                          ),
